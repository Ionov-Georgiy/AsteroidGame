...
namespace AsteroidGame.Objects
{
    class Bullet : ObjectInSpace, ICollisionable
    {
        ...
        public Bullet(int Position)
            : base(new Point(0, Position), Point.Empty, new Size(Bullet_Size_X, Bullet_Size_Y))
        {

        }

        override public void MoveInSpace(Size SpaceSize)
        {
            Position.X += Bullet_Speed;
            if (Position.X > SpaceSize.Width)
                Position.X = 0;
        }
